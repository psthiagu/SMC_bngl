...
s = SMC(config_file="config_poly.yaml")
...
def mutGaussianKeepMeans(individual, indpb, sigma_perc):
    ...
    # mu = np.array(individual)
    mu[np.isinf(mu)] = 0.0
    sigma = [i*sigma_perc if i*sigma_perc > sigma_perc else sigma_perc for i in mu]
...
toolbox.register("mutate", mutGaussianKeepMeans, indpb=0.20, sigma_perc=3.0)
